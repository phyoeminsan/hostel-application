...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
        <h2 class="fw-bold text-dark mb-0">Add New Major</h2>
    </div>
        ...
        <div class="card shadow-sm p-4 bg-white rounded">
            <div class="d-flex align-items-center p-3 mb-4 bg-primary-subtle rounded-3 border-start border-4 border-primary shadow-sm">
                ...
                <div>
                    ...
                    <small class="text-secondary">Fill in the required information below to register a new academic major.</small>
                </div>
            </div>
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('backend.majors.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="col-12">
                    <label class="form-label fw-bold">Major Name</label>
                    <input type="text" name="major_name" id="major_name" class="form-control @error('major_name') is-invalid @elseif(old('major_name')) is-valid @enderror" value="{{ old('major_name') }}">
                    @error('major_name')
